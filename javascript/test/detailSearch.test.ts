...
test("Promise resolved when request success", async () => {
  ...
  const resp = await client.detailSearch({ siteId: "EB731B7298F1E2EEDC42532CFD3B8E32" });
...
test("Promise rejected when response status code not OK", async (): Promise<void> => {
  nock("https://siteapi.cloud.huawei.com")
    ...
    .post("/mapApi/v1/siteService/searchById?key=invalidkey")
    .reply(400, error);
  await expect(client.detailSearch({ siteId: "" }, { retryOpts: { maxRetry: 3 } })).rejects.toHaveProperty(
    "returnCode",
    "010010"
  );
});
