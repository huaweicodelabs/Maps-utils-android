...
test("Promise resolved when request success", async () => {
  nock("https://siteapi.cloud.huawei.com")
    .post("/mapApi/v1/siteService/geocode?key=invalidkey")
    .reply(200, { returnCode: "0" });
  const resp = await client.geocode({ address: "somewhere" });
...
test("Promise rejected when response status code not OK", async (): Promise<void> => {
  nock("https://siteapi.cloud.huawei.com")
    ...
    .post("/mapApi/v1/siteService/geocode?key=invalidkey")
    .reply(400, error);
  await expect(client.geocode({ address: "" }, { retryOpts: { maxRetry: 3 } })).rejects.toHaveProperty(
    "returnCode",
    "010010"
  );
});
