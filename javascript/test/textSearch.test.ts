...
test("Promise resolved when request success", async () => {
  nock("https://siteapi.cloud.huawei.com")
    .post("/mapApi/v1/siteService/searchByText?key=invalidkey")
    .reply(200, { returnCode: "0" });
  const resp = await client.textSearch({ query: "restaurant" });
...
test("Promise rejected when response status code not OK", async (): Promise<void> => {
  nock("https://siteapi.cloud.huawei.com")
    ...
    .post("/mapApi/v1/siteService/searchByText?key=invalidkey")
    .reply(400, error);
  await expect(client.textSearch({ query: "" }, { retryOpts: { maxRetry: 3 } })).rejects.toHaveProperty(
    "returnCode",
    "010010"
  );
});
